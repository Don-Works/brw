...
func TestMergeCrossOriginFramesTranslatesToTopLevelCenter(t *testing.T) {
	...
	n, read := MergeCrossOriginFrames(snap, frames)
	if n != 1 {
	...
	if !read[0] || len(read) != 1 {
		t.Fatalf("expected box 0 reported as read, got %v", read)
	}
}
...
func TestMergeCrossOriginFramesNoMetadataBoxUsesZeroOffset(t *testing.T) {
	...
	if n, _ := MergeCrossOriginFrames(snap, frames); n != 1 {
		t.Fatalf("expected 1 merged element, got %d", n)
	}
	got := snap.Elements[0]
...
func TestMergeCrossOriginFramesEmptyIsNoOp(t *testing.T) {
	...
	if n, _ := MergeCrossOriginFrames(snap, nil); n != 0 {
...
func TestMergeCrossOriginFramesMatchesBoxByOrigin(t *testing.T) {
	...
	_, read := MergeCrossOriginFrames(snap, frames)
	...
	// The ref index is the BOX index, not the extraction order: b.test was
	// extracted first but is box 1, so its control is f1:… and brw_frame "f1"
	// resolves to the same frame the ref came from.
	if snap.Elements[0].Ref != "f1:e0" {
		t.Fatalf("b.test control should be numbered by its box (f1:e0), got %q", snap.Elements[0].Ref)
	}
	if snap.Elements[1].Ref != "f0:e0" {
		t.Fatalf("a.test control should be numbered by its box (f0:e0), got %q", snap.Elements[1].Ref)
	}
	if !read[0] || !read[1] || len(read) != 2 {
		t.Fatalf("expected both boxes reported as read, got %v", read)
	}
}
