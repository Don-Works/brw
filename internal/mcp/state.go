...
var errSessionStateUnavailable = errors.New("this browser transport does not support session snapshots: brw_state reads and writes cookies at the browser-context level, which needs the CDP browser target — it is refused on a browser you are personally signed into whatever its CDP can do, and refused again when the browser is on another machine, because the snapshot store holds sessions a human signed into on this one")
