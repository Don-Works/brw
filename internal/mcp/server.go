...
func tools() []map[string]any {
	catalogue := []map[string]any{
		...
		tool("brw_wait_for", "Wait for page readiness, a URL/title/text substring, a ref or CSS selector, the page's own JS predicate (fn:), a JavaScript dialog, or a file download to complete. Returns {ok, condition, resolved_by, waited_ms, wakeups}. resolved_by says what answered: \"event\" is a browser event subscription that delivered the signal or had already recorded it (no round trip and no latency floor — load, dialog and download resolve this way on the direct-CDP transport), \"script\" is one awaited in-page promise that resolves on the DOM mutation or navigation satisfying it, \"poll\" is a re-ask on a timer, which is how the extension transport answers dialog and download waits because it has no debugger attached to subscribe with. Every condition works on both transports; only the cost differs. When brw is proxying a remote brwd too old to report an outcome, resolved_by and wakeups are absent and the rest still holds.", object(map[string]any{
