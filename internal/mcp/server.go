...
func tools() []map[string]any {
	catalogue := []map[string]any{
		...
		tool("brw_open_incognito", "Open a URL in a brand-new INCOGNITO browser context: a fully isolated session with its own cookies, storage, and cache that shares nothing with the normal profile or other contexts (the CDP equivalent of an incognito window). Needs the CDP browser target, which every CDP transport has: the extension-bridge transport cannot create an isolated context and returns an error. Returns the new tab including its context_id. WHEN DONE, call brw_close_context with that context_id to dispose the whole context (closes every tab in it and discards its data). NOT ON THE EXTENSION BRIDGE: there (driving the user's existing signed-in Chrome through the extension) this returns an error — use a direct-CDP profile, or the chrome-opt-in-cdp transport. Ideal for clean-room / logged-out internal testing.", object(map[string]any{
			"url": stringSchema("URL to open in the new incognito context. Scheme defaults to https."),
		}, []string{"url"})),
		tool("brw_close_context", "Dispose an incognito browser context created by brw_open_incognito: closes every tab inside it and discards its isolated cookies/storage. Pass the context_id returned by brw_open_incognito. Not on the extension-bridge transport.", object(map[string]any{
		...
		tool("brw_set_locale", "Override the locale and time zone a tab uses for Intl formatting, Date.toString and the Accept-Language header, so they resolve where you chose instead of the host. Set locale (BCP 47, e.g. en-GB), timezone (IANA, e.g. Europe/London), or both; the fields are independent, so naming only a timezone leaves the locale alone. clear:true restores the host's own locale AND timezone, because CDP exposes no way to read back which override is in force. Intl and Date follow immediately; a document that already formatted something needs nothing, but navigator.language is NOT driven by this override on every browser build — for navigator.language and navigator.languages use brw_set_user_agent with accept_language (they are deliberately separate tools because they are separate CDP overrides). Returns {ok, tab_id, locale:{locale, timezone}, message}. NOT ON THE EXTENSION BRIDGE: it is a DevTools Protocol session override, so there it returns a named capability error.", object(map[string]any{
			"locale":   stringSchema("BCP 47 language tag for Intl and Accept-Language, for example en-GB, fr-FR, or de-DE. Omit to leave the locale unchanged. Does not set navigator.language on every build; use brw_set_user_agent accept_language for that."),
		...
		tool("brw_read", "Return semantic page content: main text, headings, links, forms, tables, and metadata. Prose is bounded by default and paged via next_offset — a long article is several cheap reads, not one huge one. Narrow with include to skip what you do not need (include:[\"headings\",\"links\"] is a cheap page map).", object(map[string]any{
			...
			"include": map[string]any{
				...
				"description": "Sections to return, as an array of strings or a comma-separated string (include:[\"headings\",\"links\"] and include:\"headings,links\" are equivalent). Valid names: main, headings, links, forms, tables, metadata. Omit for all of them.",
			},
		...
		tool("brw_snapshot", "Return interactive controls with stable refs. Defaults to a bounded visible/actionable viewport frontier; use mode:\"all\" for full-page debugging (returns every matching element including offscreen/hidden controls — useful for comprehensive page analysis), and add include_hidden:true only when hidden inputs are needed. Metadata includes total_candidates for the full count before filtering.", object(map[string]any{
			...
			"include_frames":       boolSchema("Read CROSS-ORIGIN iframes (embedded editors/widgets the normal walk cannot enter). Their controls come back as elements with source:[\"frame\"] and refs f<i>:<ref>, read by the same walker as the rest of the page, plus a cx/cy click point. On the direct-CDP backend those refs RESOLVE: pass one to brw_click and the click lands in that frame; every other ref-taking verb refuses it by name rather than acting on something else. On the extension bridge they are coordinate-only — use brw_click_xy at cx/cy. A frame with no CDP target of its own is surfaced as one clickable box, ref f<i>. Same-origin iframes are always read regardless. Off by default."),
		...
		tool("brw_find", "Find matching semantic element refs without dumping the full page. Pass action to LOCATE AND ACT in one call (click, fill, type, select, hover) instead of a find followed by a brw_click. The search must resolve to EXACTLY ONE element or the call fails and lists the rivals (ref, role, name) so you can act on the right one without searching again — it never picks the best of several. limit is ignored when action is set, so a narrow limit cannot manufacture uniqueness; narrow with role or exact:true instead. Returns {matched, action, result:<the usual post-action observation>}. Available as a find_act step in brw_batch and brw_plan on every transport.", object(map[string]any{
			"tab_id":         stringSchema("Tab id from brw_list_tabs. Omit for the active tab."),
		...
		tool("brw_route", "Intercept matching requests and answer them without touching the network: mock an API response, force an error status, abort a request entirely (analytics, a slow third party), or REPLAY a whole recorded HAR so a page loads from a fixture instead of from a live backend. pattern is a URL glob where * matches any run of characters; a pattern with no * matches as a prefix. First matching route wins, so add specific rules before general ones. Routes never widen what the page may reach: a request the navigation policy forbids stays blocked. Active routes are reported by brw_observe so mocked traffic is never invisible in the transcript.\n\naction=replay takes a har_artifact_id from brw_artifact_capture{kind:\"har\"} and answers each matching request with the exchange recorded for it. A brw HAR records FETCH AND XHR ONLY (it comes from the in-page wrappers), so a replay answers those and nothing else: the document, scripts, stylesheets and images always load from the network whatever pattern says, and the count let through that way is reported as fixture.not_replayable. on_miss therefore scopes the page's API calls, not the page: passthrough (the default) sends an unrecorded call to the real network, fail refuses it; either way the unmatched method and URL are recorded in the route's fixture.misses and reported by brw_observe. match lists the request properties an entry has to agree on and defaults to [method,url]; add \"body\" only for a fixture captured with redaction:\"none\" whose recordings differ by request body, because an ordinary capture stores \"[redacted by brw]\" in place of every request body and a body-keyed replay of one is refused at install time; so is one whose request bodies ran over the 2 KiB capture cap, since the recording then holds a prefix the live request can never equal. Entries are consumed in recorded order, so a URL recorded twice with different answers replays in sequence; once every matching entry is used the first answers again. Recorded bodies are 2 KiB capture snippets, requests and responses alike: a larger response replays clipped, and the reply reports truncated_entries and served_truncated rather than passing a half body off as a whole one. Redaction happens at RECORD time and there is no un-redacted replay mode.\n\nTRANSPORT: fulfill and replay need response bodies from the DevTools Fetch domain, so they run on every CDP transport; on the extension-bridge transport (the user's signed-in Chrome through the extension) they return a named capability error and behaviour=abort, enforced with a declarativeNetRequest session rule, is what works there. There is no redirect behaviour on ANY transport: behaviour=\"redirect\" returns a refusal naming the reason rather than a rule that quietly does nothing, because a declarativeNetRequest redirect needs host permissions brw's extension does not hold, and a URL rewritten with Fetch.continueRequest is never offered to the containment boundary again. Mock the endpoint with fulfill, or point the page at the other server.", object(map[string]any{
			...
			"pattern":         stringSchema("URL glob, e.g. https://api.example.com/v1/* — required for add, optional for clear, and for replay it narrows which fetch/XHR requests come from the HAR (default *, every recordable request)."),
			...
			"match": map[string]any{
				"type":        "array",
				"description": "For action=replay: which request properties a recorded entry must agree on. Defaults to [method,url]. \"body\" needs a capture taken with redaction:\"none\" whose request bodies fit under the 2 KiB capture cap; a redacted or clipped one is refused, because neither can ever match. A request body Chrome withholds from the interception is reported as a fixture miss rather than matched as an empty body.",
				"items":       stringEnumSchema("Match key.", browser.HARMatchMethod, browser.HARMatchURL, browser.HARMatchBody),
			},
			"on_miss": stringEnumSchema("For action=replay: what a fetch or XHR within the pattern that the HAR does not hold does. passthrough (default) sends it to the network; fail refuses it. Both record the method and URL. Neither applies to the document or the page's assets, which a brw HAR never contains.", browser.HARMissPassthrough, browser.HARMissFail),
			"tab_id":  stringSchema("Tab id from brw_list_tabs. Omit for the active tab."),
		}, []string{"action"})),
		...
		tool("brw_call_page_tool", "Invoke a WebMCP page tool by name with arguments matching its inputSchema (discover them via brw_page_tools). Use this instead of clicking through the UI when the page declares a tool for the task. Waits up to timeout_ms (default 30000) and returns {ok:true, status:\"done\", id, tab_id, result}, or {ok:false, status} for failed/cancelled/not_found/invalid_input with an error (not_found means list the page's tools again, invalid_input means fix the arguments). For work the page runs slowly — an export, a checkout, a remote search — pass detach:true to get {ok:true, id, status:\"running\", tab_id} back immediately and collect it later with brw_page_tool_result; a tool that fails the moment it is called answers {ok:false, status:\"failed\", error} even when detached, so there is nothing to collect. A waited call that outlasts timeout_ms returns timed_out:true and the SAME id, and one cut short another way (a cancelled request, a closed tab) returns interrupted:true and that id, so the invocation is never abandoned, only stopped being waited on. Arguments are capped at 64KB and refused (never truncated) above it: pass a URL or record id the tool can fetch instead of inlining a payload. The page decides the result's size, so the report is windowed like brw_evaluate — the leading 64KB unless you pass offset/max_bytes, truncation marked explicitly.", object(map[string]any{
			...
			"timeout_ms":     integerSchema("How long to wait for a non-detached invocation. Defaults to 30000, capped at 600000. Negative is refused: omit it for the default."),
			...
			"offset":         map[string]any{"type": "integer", "description": "Byte offset into the serialized report to start returning from. Defaults to 0. Use with the marker on a truncated response to page forward."},
			"max_bytes":      map[string]any{"type": "integer", "description": "Maximum bytes of the serialized report to return in this call. Defaults to 65536. A page tool decides its own result size, so a large one is truncated (with a marker) rather than dropped."},
			"tab_id":         stringSchema("Tab id from brw_list_tabs. Omit for the active tab."),
		}, []string{"name"})),
		tool("brw_page_tool_result", "Collect a detached WebMCP invocation by the id brw_call_page_tool returned. With no timeout_ms it reports the current state in one round trip — {status:\"running\"} while the tool works, {ok:true, status:\"done\", result} once it finishes, or status failed/cancelled with an error — so you can interleave polls with other work. Pass timeout_ms to block until it settles instead. The poll looks in the active tab unless you pass tab_id, so pass back the tab_id the invocation reported whenever the page tool may have opened or focused another tab. status:\"lost\" means no document in the polled tab holds that id — its document navigated away before the tool finished, or the poll landed on the wrong tab — and is reported as soon as it is noticed rather than waited out; status:\"unknown\" means the polled document is the one that minted ids and never minted that one. A finished result stays collectable for five minutes, windowed like brw_evaluate via offset/max_bytes.", object(map[string]any{
			"invocation_id": stringSchema("Invocation id from brw_call_page_tool."),
			"timeout_ms":    integerSchema("Wait up to this long for the invocation to settle. Omit for a single non-blocking read. Negative is refused. Capped at 600000."),
			"offset":        map[string]any{"type": "integer", "description": "Byte offset into the serialized report to start returning from. Defaults to 0. Use with the marker on a truncated response to page forward."},
			"max_bytes":     map[string]any{"type": "integer", "description": "Maximum bytes of the serialized report to return in this call. Defaults to 65536. A page tool decides its own result size, so a large one is truncated (with a marker) rather than dropped."},
			"tab_id":        stringSchema("Tab id the invocation reported. Omit for the active tab; pass it when the page tool may have moved the active tab."),
		}, []string{"invocation_id"})),
		tool("brw_page_tool_cancel", "Stop waiting on a WebMCP invocation and signal the page tool to abort. brw passes every invocation an AbortSignal, so a cooperating tool stops its own work; one that ignores the signal keeps running in the page, but its result is no longer awaited and later polls report status:\"cancelled\". Cancelling an invocation that already finished is not an error — it returns that outcome with cancelled:false, windowed like brw_evaluate via offset/max_bytes. It cancels in the active tab unless you pass the tab_id the invocation reported.", object(map[string]any{
			"invocation_id": stringSchema("Invocation id from brw_call_page_tool."),
			"offset":        map[string]any{"type": "integer", "description": "Byte offset into the serialized report to start returning from. Defaults to 0. Use with the marker on a truncated response to page forward."},
			"max_bytes":     map[string]any{"type": "integer", "description": "Maximum bytes of the serialized report to return in this call. Defaults to 65536. A page tool decides its own result size, so a large one is truncated (with a marker) rather than dropped."},
			"tab_id":        stringSchema("Tab id the invocation reported. Omit for the active tab; pass it when the page tool may have moved the active tab."),
		}, []string{"invocation_id"})),
		tool("brw_group_tabs", "Group tabs into a named Chrome tab group, or move them into an existing group_id. Extension-bridge transport only: Chrome tab groups are an extension API with no DevTools Protocol equivalent, so no CDP transport can create them.", object(map[string]any{
		...
		tool("brw_ungroup_tabs", "Remove tabs from their Chrome tab group. Extension-bridge transport only: Chrome tab groups are an extension API with no DevTools Protocol equivalent, so no CDP transport can change them.", object(map[string]any{
		...
		tool("brw_notify", "Raise a desktop notification to pull the human operator back at a hand-off point (needs_input for MFA/CAPTCHA/purchase confirmation), on completion (done), or on failure (error) — useful when the user has tabbed away. With the Chrome extension bridge this uses chrome.notifications and surfaces even when the tab is backgrounded; without the extension bridge it falls back to the in-page Notification API (best-effort, subject to page focus/permission). The result reports the honest delivery channel (extension, page, or unavailable).", object(map[string]any{
