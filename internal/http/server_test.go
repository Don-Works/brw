...
func TestFillForwardsBody(t *testing.T) {
	ctrl := &fakeController{snap: sampleSnapshot()}
	server := New("", ctrl)
	body := bytes.NewBufferString(`{"query":"email","role":"textbox","text":"form@example.invalid","replace":true}`)
	...
	if ctrl.fillOpts.Query != "email" || ctrl.fillOpts.Role != "textbox" || ctrl.fillOpts.Text != "form@example.invalid" || !ctrl.fillOpts.Replace {
