...
func (c *Controller) AddInitScript(ctx context.Context, opts browser.InitScriptOptions) (browser.InitScriptResult, error) {
	var out browser.InitScriptResult
	err := c.post(ctx, "/api/page/init_script", map[string]any{"action": "add", "source": opts.Source, "origin": opts.Origin, "tab_id": opts.TabID}, &out)
	return out, err
}

func (c *Controller) RemoveInitScript(ctx context.Context, opts browser.InitScriptRemoveOptions) (browser.InitScriptResult, error) {
	var out browser.InitScriptResult
	err := c.post(ctx, "/api/page/init_script", map[string]any{"action": "remove", "id": opts.ID, "tab_id": opts.TabID}, &out)
	return out, err
}

func (c *Controller) ListInitScripts(ctx context.Context, tabID string) (browser.InitScriptResult, error) {
	var out browser.InitScriptResult
	err := c.post(ctx, "/api/page/init_script", map[string]any{"action": "list", "tab_id": tabID}, &out)
	return out, err
}
