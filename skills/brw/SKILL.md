...
- `stored_in` names where the baseline went. A recipe the private provider owns keeps its baselines with the provider — the capture is of a page that recipe reached, so it is the provider's kind of secret — and `--baseline-root` holds public fixtures and anything no provider claims. You do not choose: the recipe and the page decide. `recipe_digest` is your argument, so it is cross-checked against the tab: a digest the provider does not own, on a page the provider has a recipe for, is refused rather than written locally. A daemon whose provider cannot be reached fails the call rather than writing a private page's screenshot to the local root, and one started with `--upstream-http` refuses the provider's own baselines by name — take those on the browser-host daemon.
