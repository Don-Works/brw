...
**Faking the page's surroundings.** `brw_set_geolocation`, `brw_set_network_conditions`, `brw_emulate_media`, `brw_set_extra_headers`, `brw_set_user_agent` and `brw_authenticate` are not on the extension bridge; each returns a named capability error there and is not advertised.
