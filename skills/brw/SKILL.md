...
- `brw_plan({steps})` is the older sibling: it adds `snapshot`/`read` steps and `expect_ref`/`expect_role` guards but returns a result per step. Its intermediate steps report a minimal observation and its last step the full one. Prefer `brw_batch` unless you need a mid-flow snapshot.
