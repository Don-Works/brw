...
- `brw_set_locale({locale?,timezone?,clear?,tab_id?})` — the BCP 47 language and IANA zone the renderer reports, so `Intl` formatting, `Date.toString` and `Accept-Language` use a locale you chose. The two fields are independent; `clear:true` restores the host's own locale and zone. A document that already parsed its language needs a reload.
