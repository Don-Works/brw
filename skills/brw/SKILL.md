...
daemon (75 on the extension bridge, which cannot serve the other fifteen); the
...
- `stored_in` names where the baseline went. A recipe the private provider owns keeps its baselines with the provider — the capture is of a page that recipe reached, so it is the provider's kind of secret — and `--baseline-root` holds public fixtures and anything no provider claims. You do not choose: the digest decides. A daemon whose provider cannot be reached fails the call rather than writing a private page's screenshot to the local root.
